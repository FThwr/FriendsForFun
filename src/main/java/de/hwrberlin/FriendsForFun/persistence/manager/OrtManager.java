...
public class OrtManager extends AbstractEntityManager {
	...
	public Ort getOrte(String bez_ort, String plz, String strasse) throws NoResultException {
		...
		try {
			return (Ort) em.createQuery(
					"SELECT o FROM Ort o WHERE o.bez_ort = :oBez_ort AND o.plz = :oPlz AND o.strasse = :oStrasse")
					.setParameter("oBez_ort", bez_ort).setParameter("oPlz", plz).setParameter("oStrasse", strasse)
